...
function wt(id: string, extra: Partial<Worktree> = {}): Worktree {
  return {
    ...
    metadata: { display_name: null, project: null, state: null, priority: null, tags: [] }, last_active_ms: null, first_seen_ms: null, archived_at_ms: null, archiving: false, town_slug: null, tab_count: 0, pane_count: 0, ...extra,
  };
}
...
const list = [
  wt("a", { metadata: { display_name: null, project: "LR", state: "merged", priority: 2, tags: ["lr"] } }),
  wt("b", { metadata: { display_name: null, project: null, state: "exploring", priority: 1, tags: [] }, last_active_ms: 5 }),
...
describe("home query", () => {
  ...
  it("sorts by priority, attention, created, state", () => {
  ...
  it("groups by state in config order with empty columns kept and no state last", () => {
    ...
    expect(groupWorktrees([wt("x", { metadata: { display_name: null, project: null, state: "ghost", priority: null, tags: [] } })], "state", ctx).map((g) => g.key)).toEqual(["Exploring", "Active", "Merged", "ghost"]);
  });
});
