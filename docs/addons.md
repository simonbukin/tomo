...
| `hook_events` | `config::check`, when a hook in `config.toml` names its event | Each addon lists the hook events it fires (Actions, Runtime, Agentation), so a hook can name them. The addon puts its own data on the event under its own key in `HookEvent.addons`, such as `action`. |
