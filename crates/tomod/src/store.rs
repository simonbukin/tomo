...
use tomo_proto::{ActivityEvent, AgentKind, AttentionItem, AttentionKind, AttentionLevel, Id, LayoutNode, PaneKind, TownUnlock, WorktreeMetadata};
...
impl Store {
    ...
    pub fn activity_insert(&self, e: &ActivityEvent) -> Result<()> {
        let kind = serde_json::to_value(e.kind)?.as_str().unwrap_or_default().to_string();
        self.conn.execute(
            ...
            params![e.id, kind, e.occurred_at_ms as i64, e.worktree_id, e.pane_id, e.agent_kind.map(agent_kind_str), e.title, e.detail, e.payload.to_string(), e.attention_id],
        )?;
        Ok(())
    }
