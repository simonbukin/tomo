...
impl Daemon {
    ...
    fn create_browser_pane(inner: &mut Inner, tab_id: &str, worktree_id: &str, cwd: PathBuf, url: String) -> Result<Id> {
        ...
        inner.panes.insert(
            id.clone(),
            PaneState { row, pty: None, origin: PaneOrigin::Live, exit_code: None, process_title: None, process_cmd: None, pending_line: None, last_output_ms: 0, scrollback: Scrollback::default() },
        );
        let mut ev = events::envelope(inner, "pane.created", Some(worktree_id));
    ...
    async fn handle_inner(self: &Arc<Self>, client_id: u64, call: Call) -> Result<Value, RpcError> {
        match call {
            ...
            Call::RuntimeList { .. } | Call::ActivityList(_) | Call::CheckpointCreate(_) | Call::CheckpointResolve { .. } | Call::UsageGet { .. } => {
            ...
            Call::AnnotationsSend { pane_id, bundle } => {
                ...
                activity::record(&mut inner, event.clone());
